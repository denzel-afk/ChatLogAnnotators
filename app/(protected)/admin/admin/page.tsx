...
export default function AdminPage() {
    ...
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        const conversationData = {
          ...
          last_interact: {
            text: chatlog.last_interact?.text || "Unknown Last Interaction",
            timestamp: chatlog.last_interact?.timestamp || 0,
          },
  ...
  useEffect(() => {
    fetchFirstConversation().then((data) => {
  ...
  const handleAddAnnotation = () => {
    ...
    fetch(`/api/conversations`, {
      ...
      .then(() => {
        ...
        fetchFirstConversation().then((data: Conversation | null) =>
          setConversation(data)
        );
      })
      .catch((err) => {
  ...
  const handleDeleteAnnotation = (annotationId: string) => {
    ...
    fetch(`/api/conversations/`, {
      ...
      .then(() => {
        ...
        fetchFirstConversation().then((data) => setConversation(data));
      })
      .catch((err) => {
  ...
  const handleEditllMessageAnnotation = (
    ...
    if (!newAnnotation?._id || !newAnnotation.title || !newAnnotation.type) {
      toast.error(
  ...
  return (
    <div className="p-4 bg-background text-foreground h-full overflow-auto">
      ...
      >
        Add Annotation for Conversations
      </button>
